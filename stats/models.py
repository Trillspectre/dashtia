...
class Statistic(models.Model):
    ...
    # Note: min_value and max_value removed intentionally.
    # Validation has been moved to callers where appropriate.
    CHART_TYPE_CHOICES = [
        ("pie", "Pie Chart"),
        ("bar", "Bar Chart"),
    ...
    chart_type = models.CharField(
        max_length=20, choices=CHART_TYPE_CHOICES, default="pie"
    )
    ...
    teams = models.ManyToManyField(
        ...
        related_name="statistics",
        help_text=(
            "Teams that can view this KPI (when visibility is set to 'team')"
        ),
    )
    ...
    is_active = models.BooleanField(
        default=True,
        help_text=(
            "Soft-delete flag. Inactive items are hidden from lists."
        ),
    )
...
class DataItem(models.Model):
    ...
    def clean(self):
        if (
            self.statistic.min_value is not None
            and self.value < self.statistic.min_value
        ):
            raise ValidationError(
                f"Value must be at least {self.statistic.min_value}"
            )
        if (
            self.statistic.max_value is not None
            and self.value > self.statistic.max_value
        ):
            raise ValidationError(
                f"Value must not exceed {self.statistic.max_value}"
            )
...
class KpiDeletion(models.Model):
    ...
    statistic = models.ForeignKey(
        Statistic, on_delete=models.CASCADE, related_name="deletions"
    )
    ...
    def __str__(self):
        return (
            f"Deletion of {self.statistic.name} by {self.deleted_by} "
            f"at {self.deleted_at}"
        )

...
class KPIPermissionMixin(LoginRequiredMixin):
    ...
    def can_view_kpi(self, kpi):
        ...
        if kpi.visibility == "public":
            ...
            user_teams = user.team_memberships.values_list(
                "team_id", flat=True
            )
            kpi_teams = kpi.teams.values_list("id", flat=True)
            return bool(set(user_teams) & set(kpi_teams))
        ...
        if new_stat:
            obj, created = Statistic.objects.get_or_create(
                ...
                defaults={
                    ...
                    "unit_type": unit_type,
                    "custom_unit": (
                        custom_unit if unit_type == "custom" else ""
                    ),
            ...
            if created:
                ...
                messages.warning(
                    request, f'KPI "{obj.name}" already exists!'
                )
        # Stay on same page instead of redirecting
        return self.get(request, *args, **kwargs)

    def get_context_data(self, **kwargs):
        # Provide accessible teams to the template so the create-KPI
        # form can show the teams multi-select
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_staff or user.is_superuser:
            ...
            for other_user in User.objects.exclude(id=user.id):
                user_kpi_count = Statistic.objects.filter(
                    owner=other_user
                ).count()
                if user_kpi_count > 0:
                    dropdown_options.append(
        ...
        context.update(
            {
                ...
                # do not override `user` context provided by auth
...
class StatisticDeleteView(KPIPermissionMixin, View):
    """
    Handle soft-deletion of a Statistic (KPI).
    ...
    def post(self, request, pk):
        ...
        if (
            ...
            or request.META.get("HTTP_ACCEPT", "").find("application/json")
            != -1
        ):
            return JsonResponse({"success": True, "id": obj.pk})

    # Otherwise redirect back to the main stats page
    # (normal form POST fallback)
        messages.success(
            request, f'KPI "{obj.name}" deleted'
        )
...
class DataItemDeleteView(View):
    ...
    # Permission: only staff/admin or the original owner
    # (username string) may delete
        requester = request.user
        if not (
            requester.is_staff
            or requester.is_superuser
            or item.owner == getattr(requester, "username", None)
        ):
            return JsonResponse(
                {"success": False, "error": "Permission denied"}, status=403
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == "create_team":
            ...
            try:
                ...
                messages.success(
                    request, f"Team '{team_name}' created successfully"
                )
            ...
            if created:
                messages.success(
                    request, f"{user.username} added to {team.name}"
                )
            else:
                messages.info(
                    request, f"{user.username} is already in {team.name}"
                )
    ...
    def get_form(self, form_class=None):
        ...
        if not (self.request.user.is_staff or self.request.user.is_superuser):
            if "visibility" in form.fields:
                ...
                form.fields["visibility"].help_text = (
                    "Contact admin to change visibility"
                )
...
class UserStatisticsView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        user_stats_ids = (
            DataItem.objects.filter(owner=self.request.user.username)
        ...
        return JsonResponse(
            {
                ...
                "latest_entries": [
                    {"owner": item.owner, "value": item.value, "id": item.id}
            ...
            context.update(
                {
                    ...
                    "unique_contributors": obj.data.values_list(
                        "owner", flat=True
                    )
                    .distinct()
                    .count(),
                    "average_value": obj.data.aggregate(avg=Avg("value"))[
                        "avg"
                    ],
                }
            )
        ...
        context.update(
            {
                ...
                "top_contributors": obj.data.values("owner")
                .annotate(total=Sum("value"))
                .order_by("-total")[:5],
            }
        )
